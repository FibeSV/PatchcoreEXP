    ...
    def __getitem__(self, index):
        ...
        if "good" in path:                                      # Nominal image
            ...
            if not os.path.exists(mask_path):
                mask_path = mask_path.replace( "_mask.png",".png")  
                mask_path = mask_path[:-7]+mask_path[-7:-4]+"\\000"+mask_path[-4:]
        ...
        if self.target_transform is not None:
            ...
            original_sample = self.target_transform(original_sample)

        return sample, mask[:1], sample_class, {"image":original_sample, "path":path}
